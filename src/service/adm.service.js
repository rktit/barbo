...
export const getEnterpriseForId = async (id) => {
  let result = [];
  await axios
    .get(`http://localhost:21038/api/enterprise/getById/` + id)
    .then((res) => {
      console.log("getEnterpriseForId result", res);
      result = { ...res.data.data, error: false };
    })
    .catch((error) => {
      console.log("getEnterpriseForId catch", error);
      result = { error: true };
    });
  return result;
};
